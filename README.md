...
When localization exposes feature-match images, `frame_000_matches.jpg` contains raw accepted feature matches; otherwise a placeholder image is written so the debug set remains complete.
...
- Add fixture-based visual regression checks for localization debug artifacts.
...
Localization results expose the following confidence and quality metrics in each frame record:

- `num_keypoints`: detected keypoints in the current frame.
- `num_matches`: feature matches that passed ratio filtering.
- `num_inliers`: RANSAC inlier matches.
- `inlier_ratio`: `num_inliers / num_matches`.
- `reprojection_error`: mean reprojection error for inlier matches when available.
- `message`: `SUCCESS` or a failure reason such as `FAILED_TOO_FEW_MATCHES`, `FAILED_TOO_FEW_INLIERS`, `FAILED_LOW_INLIER_RATIO`, or `FAILED_INVALID_HOMOGRAPHY`.
...
When localization fails, the pipeline continues and records the failed localization status in the frame JSON. If detection succeeds but localization fails, detections remain in the output with `localized: false`.

Debug mode now writes feature match visualizations when the localization method exposes them:

```text
output/debug/
  frame_000_matches.jpg
  frame_000_inlier_matches.jpg
```

If raw feature matches are not available for the selected localization method, a placeholder match image is still written.
