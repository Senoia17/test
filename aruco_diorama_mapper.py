...
map coordinate system. If IDs are not provided, the script orders the four
detected marker centers geometrically, which works when the field is visible as a
convex rectangle in the image.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=True,
        help="Real-world rectangular arena size in meters.",
    )
    parser.add_argument(
        "--pixels-per-meter",
        type=int,
    ...
    parser.add_argument(
        "--debug-image",
        type=Path,
        help="Optional image path showing detected markers and selected corner centers.",
    )
...
def marker_centers(corners: Iterable[np.ndarray], ids: Iterable[int]) -> dict[int, np.ndarray]:
    ...
    for marker_corners, marker_id in zip(corners, ids):
        ...
        centers[int(marker_id)] = points.mean(axis=0)
    return centers
...
def select_source_points(centers: dict[int, np.ndarray], marker_ids: list[int] | None) -> tuple[np.ndarray, list[int]]:
    if marker_ids:
        missing = [marker_id for marker_id in marker_ids if marker_id not in centers]
        if missing:
            raise RuntimeError(f"Required corner marker IDs were not detected: {missing}")
        return np.array([centers[marker_id] for marker_id in marker_ids], dtype=np.float32), marker_ids
    ...
    ordered = order_points_geometrically(points)
    ordered_ids = [ids[int(np.argmin(np.linalg.norm(points - point, axis=1)))] for point in ordered]
    return ordered, ordered_ids


def create_map(image: np.ndarray, source_points: np.ndarray, width_m: float, height_m: float, ppm: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    output_width = int(round(width_m * ppm))
    output_height = int(round(height_m * ppm))
    if output_width <= 0 or output_height <= 0:
        raise ValueError("Output dimensions must be positive. Check --arena-size and --pixels-per-meter.")
...
def write_debug_image(image: np.ndarray, corners: list[np.ndarray], ids: np.ndarray, source_points: np.ndarray, path: Path) -> None:
    debug = image.copy()
...
def main() -> None:
    ...
    source_points, selected_ids = select_source_points(centers, args.marker_ids)
